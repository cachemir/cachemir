...
const tempKeyTTL = 5 * time.Second
...
func main() {
	...
	runStringOperations(c)
	runCounterOperations(c)
	runExpirationDemo(c)
	runHashOperations(c)
	runListOperations(c)
	runSetOperations(c)
...
func runStringOperations(c *client.Client) {
	fmt.Println("\n--- String Operations ---")
...
func runCounterOperations(c *client.Client) {
	fmt.Println("\n--- Counter Operations ---")
...
func runExpirationDemo(c *client.Client) {
	fmt.Println("\n--- Expiration ---")

	if err := c.Set("temp_key", "temp_value", tempKeyTTL); err != nil {
...
func runHashOperations(c *client.Client) {
	fmt.Println("\n--- Hash Operations ---")
...
func runListOperations(c *client.Client) {
	fmt.Println("\n--- List Operations ---")
...
func runSetOperations(c *client.Client) {
	fmt.Println("\n--- Set Operations ---")
...
func runCleanup(c *client.Client) {
	fmt.Println("\n--- Cleanup ---")
